# ===========================================================================
# Name        : EDITED to include indexing main_susquehanna.py
...
#TODO: note that adding index col in _solution.csv will change which rows get optimized/edited in later notebooks.
...
# this function opens and writes solutions variables (simulation solutions) to the named files
def store_results(algorithm, track_progress, output_dir, rbf_name, seed_id):
    ...
    with open(
        ...
        for index, solution in enumerate(algorithm.result):
            writer.writerow([index] + solution.objectives)
    ...
    with open(
        ...
        writer = csv.writer(f)
        for index, solution in enumerate(algorithm.result):
            writer.writerow(([index] + solution.variables)

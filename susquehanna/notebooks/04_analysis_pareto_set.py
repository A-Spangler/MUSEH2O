...
'''
#geerate min/max parallel plots
for entry in rbfs:
    name = entry.__name__
...
for entry in rbfs:
    ...
    plt.show()
'''
#plot for only one RBF, specify the first RBF entry to use
entry = rbfs[1]
name = entry.__name__
...
# Assuming `get_limits` and `ParallelAxes` are defined functions/classes
limits = get_limits(df_rbf)
axes = ParallelAxes(limits, rot=0)
axes.plot(df_rbf, label=name, lw=0.5, color="grey", alpha=0.4)
...
'''
#plots boxplots showing performance
import seaborn as sns
